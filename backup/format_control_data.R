  ...
  out <- tryCatch(
    {
      ...
      if(is.na(matched_col_names) || is.na(matching_entry_indices)){
        comments <- paste0(comments,"matching_entry_indices or matched_col_names
        has returned NA, indicating a fatal error in check_vector_entries_match")
      }
...
check_vector_entries_match <- function(current_vec, target_vec){
  ...
  out <- tryCatch(
    ...
    current_vec <- new_cull_data_df
    target_vec <- cull_legacy_df
